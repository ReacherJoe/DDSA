...
void record_data();
void retrieve_data();
void data_all_output();
void record_transaction();
void my_privilege_for_admin(int user_id);
void account_control();


int line_count_from_file(char* filename);
...
void login(){
    ...
    if(g_login_check != -1){
        ...
        printf("LoginSuccess!\n Your Id= %d\n",g_login_check);
        if (g_login_check == 1 ){
            my_privilege_for_admin(g_login_check);
        }else if (g_login_check == 2){
            my_privilege_for_admin(g_login_check);
        }else if (g_login_check == 3){
            my_privilege_for_admin(g_login_check);
        }else {
...
void registration(){
    ...
    data[g_userCount].user_id = g_userCount+1;
    g_userCount++;
...
void TransferASM (int user_id){
  user_id -=1;
    ...
    scanf(" %[^\n]",&email_ASM);
       ...
       int receiver_id = -1;
    for (int i = 0; i < g_userCount; i++) {
        if (compare_two_char_array(data[i].user_email, email_ASM) == 1) {
            receiver_id = i;
            break;
        }
    }
        ...
        if (data[user_id].ASM_point < amount_transfer) {
            printf("you point is %d no enough....",data[user_id].ASM_point);
            
        }else {
             if (receiver_id != -1) {
                data[user_id].ASM_point-= amount_transfer;
                data[receiver_id].ASM_point += amount_transfer;
                printf("-------Success-------\n");
        }
...
void record_data(){
    ...
    for(int i=0; i<g_userCount; i++){
        ...
        fprintf(fptr ,"%d %s %d %s %s %d %s %d \n",data[i].user_id,data[i].user_name,
        data[i].ASM_point,data[i].user_email,data[i].user_pass,data[i].phone_number,data[i].address,data[i].postal_code);

    // for (int j = 0; j < 50; j++) {
    //         fprintf(fptr, " %d ", data[i].transactions[j]);
    //     }
...
void retrieve_data(){
    ...
    for (register int user = 0; user < line_count_from_file("database.txt"); user++)
    {
    fscanf(fptr, "%d %s %d %s %s %d %s %d",&data[user].user_id,&data[user].user_name,&data[user].ASM_point,&data[user].user_email,&data[user].user_pass,&data[user].phone_number,&data[user].address,&data[user].postal_code);
...
void data_all_output(){
    ...
    for (register int user = 0; user < g_userCount ; user++)
    {
    
    printf("%d %s %d %s %s %d %s %d",data[user].user_id, data[user].user_name, data[user].ASM_point, data[user].user_email, data[user].user_pass, data[user].phone_number, data[user].address, data[user].postal_code);

    printf("\n");
    }

}
// void record_transaction(int user_id, int amount_transfer) {
//     user_id -= 1; 
//     for (int i = 0; i < 50; i++) {
//         if (data[user_id].transactions[i] == 0) {
//             data[user_id].transactions[i] = amount_transfer;
//             break;
//         }
//     }
// }
...
void account_control(){
    printf("----This is account-control place----");
}
